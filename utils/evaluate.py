...
class EvalFmtCvtor(object):
    ...
    def prepare_gt(self,gt_graph):
        ...
        traj_cat_ids = gt_graph.traj_cat_ids  
        traj_duras = gt_graph.traj_durations  
        pred_durations = gt_graph.pred_durations.type(torch.long) 
        pred_cat_ids  = gt_graph.pred_cat_ids  


        
        pred2so_ids = torch.argmax(gt_graph.adj_matrix,dim=-1).t()  
        pred2so_catids = traj_cat_ids[pred2so_ids]
        ...
        dura_inters = dura_inters[pred2so_ids[:,0],pred2so_ids[:,1],:] 

        check_inter,mask_ = dura_intersection_ts(pred_durations,dura_inters,broadcast=False)  
        assert torch.all(check_inter == pred_durations)  
    ...
    def to_eval_format_pr(self,proposal,pr_triplet,preserve_debug_info=False):
        ...
        traj_bboxes = proposal.bboxes_list   
        durations_list = proposal.traj_durations.clone() 
        ...
        for p_id in range(n_pr):
            ...
            subject_dura = (subject_dura_[0],subject_dura_[1]+1) 
            ...
            result_per_triplet["sub_traj"] = subject_traj.cpu().numpy().tolist()   
            result_per_triplet["obj_traj"] = object_traj.cpu().numpy().tolist()
            
            if preserve_debug_info:
                result_per_triplet["triplet_tid"] = (int(subj_tid),int(pred_catid),int(obj_tid)) 
                result_per_triplet["ori_sub_traj"] = ori_sub_traj.cpu().numpy().tolist() 
    ...
    def to_eval_format_pr_wo_cutoff(self,proposal,pr_triplet,preserve_debug_info=False,use_pku=False):
        ...
        for p_id in range(n_pr):
            ...
            result_per_triplet["duration"] = dura_   
            result_per_triplet["score"] = float(pr_score[p_id])
            result_per_triplet["sub_traj"] = ori_sub_traj.cpu().tolist()  
    ...
    def to_eval_format_gt(self,gt_graph):
        ...
        traj_bboxes = gt_graph.traj_bboxes 
        ...
        for g_id in range(n_gt):
            ...
            result_per_triplet["duration"] = dura_spo  
            result_per_triplet["sub_traj"] = subject_traj.cpu().numpy().tolist()   
            result_per_triplet["obj_traj"] = object_traj.cpu().numpy().tolist()
    ...
    def to_eval_format_gt_old(self,gt_graph):
        ...
        for p_id in range(gt_graph.num_preds):
            s_id = torch.argmax(gt_graph.adj_matrix[0,p_id,:]) 
            o_id = torch.argmax(gt_graph.adj_matrix[1,p_id,:]) 
            ...
            duration_so = dura_intersection(subject_dura,object_dura)  
            if duration_so == None:
                continue
            else:
                duration_spo = dura_intersection(duration_so,pred_dura)
                if duration_spo == None:
                    duration_spo = duration_so   
                else:
                    pass
            ...
            result_per_triplet["duration"] = duration_spo  
            result_per_triplet["sub_traj"] = subject_traj.cpu().numpy().tolist() 
            result_per_triplet["obj_traj"] = object_traj.cpu().numpy().tolist()
